...
const initialNFTs = [
	{
		...
		issuer: "Hardhat",
		image:
			"https://moralis.io/wp-content/uploads/web3wiki/24hardhat/6381641b6a60932fb3c3c2d9_crsLQ2lVok-0X37hZ_7RSl62vTm5GRP0Ws4xyPt4E5I.jpeg",
		date: "Aug 2024",
	},
	{
		id: 2,
...
export default function NFTCarousel() {
	const [NFTs, setNFTs] = useState<CredentialNFT[]>([]);

	useEffect(() => {
		const savedCredentials = localStorage.getItem("credentialNFTs");
		if (savedCredentials) {
			setNFTs(JSON.parse(savedCredentials));
		} else {
			// If no saved credentials, use the initial NFTs and save them to localStorage
			localStorage.setItem("credentialNFTs", JSON.stringify(initialNFTs));
			setNFTs(initialNFTs);
		}
	}, []);
